...
static vector<cockroach_type*> static_type_vec;
...
static map<string, vector<cockroach_type *>> static_routine_para_map;

static bool has_aggregates = false;
static vector<routine> static_aggregate_vec;

static bool has_aggregate_para = false;
static map<string, vector<cockroach_type *>> static_aggregate_para_map;
...
bool cockroach_type::consistent(sqltype *rvalue)
{
    cockroach_type *t = dynamic_cast<cockroach_type*>(rvalue);
    if (!t) {
...
bool schema_cockroach::is_consistent_with_basic_type(sqltype *rvalue)
{
...
schema_cockroach::schema_cockroach(string db, unsigned int port, string host, bool no_catalog)
    : cockroach_connection(db, port, host)
{
    ...
    if (has_types == false) {
        ...
        for (int i = 0; i < row_num; i++) {
            ...
            auto t = new cockroach_type(name, oid, typdelim[0], typrelid, typelem, typarray, typtype[0]);
    ...
    for (auto t : static_type_vec) {
        oid2type[t->oid_] = t;
        name2type[t->name] = t;
        // cerr << "t->name: " << t->name << ", t->oid_: " << t->oid_ << endl;
        types.push_back(t);
    }

    // cerr << "oid2type size: " << oid2type.size() << endl;
    ...
    internaltype = sqltype::get("no_internal");
    ...
    if (has_operators == false) {
        ...
        for (int i = 0; i < row_num; i++) {
            ...
            auto left_oid = atol(PQgetvalue(res, i, 1));
            auto right_oid = atol(PQgetvalue(res, i, 2));
            auto result_oid = atol(PQgetvalue(res, i, 3));

            // cerr << "left_oid: " << left_oid << ", right_oid: " << right_oid << ", result_oid: " << result_oid << endl;
    ...
    if (has_routines == false) {
        ...
        for (int i = 0; i < row_num; i++) {
            ...
            if (r_name == "crdb_internal") // administrative functions
                continue;
            ...
            if (oid2type.count(proretoid) == 0)
                continue;
            auto prorettype = oid2type[proretoid];
    ...
    if (has_routine_para == false) {
        ...
        // for (auto &proc : static_routine_vec) {
            ...
            for (int i = 0; i < row_num; i++) {
                auto para_oid = atol(PQgetvalue(res, i, 0));
                if (oid2type.count(para_oid) == 0) {
                    has_not_basic_type = true;
                    break;
                }
                auto t = oid2type[para_oid];
    ...
    if (has_aggregate_para == false) {
        ...
        // for (auto &proc : aggregates) {
            ...
            vector<cockroach_type *> para_vec;
            for (int i = 0; i < row_num; i++) {
                auto t_oid = atol(PQgetvalue(res, i, 0));
                if (oid2type.count(t_oid) == 0) {
                    has_not_basic_type = true;
                    break;
                }
...
schema_cockroach::~schema_cockroach()
{
...
cockroach_connection::~cockroach_connection()
{
    PQfinish(conn);
}

dut_cockroach::dut_cockroach(string db, unsigned int port, string host)
    : cockroach_connection(db, port, host)
{
    // string set_timeout_cmd = "SET statement_timeout = '" + to_string(TIMEOUT_SECOND) + "s';";
...
static bool is_expected_error(string error)
{
    if (error.find("violates not-null constraint") != string::npos
        ...
        || error.find("unimplemented: apply joins with subqueries") != string::npos
        || error.find("unknown salt algorithm") != string::npos
        || error.find("invalid salt algorithm") != string::npos
        || error.find("window functions are not allowed in") != string::npos
        || error.find("could not decorrelate subquery") != string::npos
        || error.find("error parsing regexp") != string::npos
        || error.find("no region set on the locality flag on this node") != string::npos
        || error.find("is not multi-region enabled") != string::npos
                    ...
                    vector<string>* env_setting_stmts)
{
    if (env_setting_stmts != NULL) {
        for (auto& set_statement : *env_setting_stmts) {
            ...
            if (status != PGRES_COMMAND_OK && status != PGRES_TUPLES_OK) {
                ...
                throw runtime_error("[COCKROACH] setting error [" + err + "]");
            }
        }
    }
    ...
    if (status != PGRES_COMMAND_OK && status != PGRES_TUPLES_OK) {
        ...
        if (is_expected_error(err))
            throw runtime_error("[COCKROACH] expected error [" + err + "]");
        else
            throw runtime_error("[COCKROACH] execution error [" + err + "]");
    }
...
void dut_cockroach::reset(void)
{
    ...
    string cancel_job = "CANCEL JOBS (SELECT job_id FROM [SHOW JOBS] WHERE status = 'paused' AND description ILIKE '%" + test_db + "%');";
    auto res = PQexec(conn, cancel_job.c_str());
    auto status = PQresultStatus(res);
    if (status != PGRES_COMMAND_OK && status != PGRES_TUPLES_OK){
        string err = PQerrorMessage(conn);
        PQclear(res);
...
void dut_cockroach::reset_to_backup(void)
{
    reset();
...
int dut_cockroach::save_backup_file(string db_name, string path)
{
    // string cp_cmd = "cp " + BK_FILE(db_name) + " " + path;
    // return system(cp_cmd.c_str());
    return 0;
}

void dut_cockroach::get_content(vector<string>& tables_name, map<string, vector<vector<string>>>& content)
{
