...
// txn blocking may break the adjacent relation of instrument statement
// strict_instrument_order
//  true: throw error if the order is broken
//  false: skip the statement if the order is broken, but do not throw error 
// when strict_instrument_order is false, some instrument dependencies will be missed 
void dependency_analyzer::build_stmt_instrument_dependency(bool strict_instrument_order)
{
    for (int i = 0; i < stmt_num; i++) {
        ...
        if (cur_usage == BEFORE_WRITE_READ) {
            if (i + 1 >= stmt_num) {
                ...
                cerr << err_info << endl;
                if (strict_instrument_order == false)
                    continue;
                throw runtime_error(err_info);
            }
...
{   
    ...
    build_stmt_instrument_dependency(strict_instrument_order);
    ...
    for (auto& row_history : h.change_history) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (op_unit.stmt_u == BEFORE_WRITE_READ) {
                auto stmt_1 = stmt_id(f_txn_id_queue, op_unit.stmt_idx);
                auto stmt_2 = stmt_id(f_txn_id_queue, op_unit.stmt_idx + 1);
                auto edge = make_pair(stmt_1, stmt_2);
                // skip the statement whose instrument dependency is not correctly built
                if (stmt_dependency_graph.count(edge) == 0 || stmt_dependency_graph[edge].count(INSTRUMENT_DEPEND) == 0)
                    continue;
    ...
    if (strict_instrument_order == false) { // should not consider predicate dependency if the order may be broken
        // if there is no VERSION_SET_READ statement, VS and OW dependency will not be built
...
// -1: cannot fine the original one, instrument dependency is not built
// other: the idx of original statement
int dependency_analyzer::check_original_stmt_pos(int instrument_idx)
{
    set<int> idx_set = get_instrumented_stmt_set(instrument_idx);
    // if the statement is inserted
    if (f_stmt_usage[instrument_idx] == BEFORE_WRITE_READ) {
        if (idx_set.count(instrument_idx + 1) > 0)
            return instrument_idx + 1;
        else
            return -1;
    } else if (f_stmt_usage[instrument_idx] == AFTER_WRITE_READ) {
        if (idx_set.count(instrument_idx - 1) > 0)
            return instrument_idx - 1;
        else
            return -1;
    } else if (f_stmt_usage[instrument_idx] == VERSION_SET_READ) {
        int normal_pos = instrument_idx + 1;
        while (1) {
            if (normal_pos >= stmt_num)
                return -1;  
            if (f_stmt_usage[normal_pos] == SELECT_READ ||
                    f_stmt_usage[normal_pos] == UPDATE_WRITE ||
                    f_stmt_usage[normal_pos] == INSERT_WRITE ||
                    f_stmt_usage[normal_pos] == DELETE_WRITE) {
                break;
            }
            normal_pos++;    
        }
        if (idx_set.count(normal_pos) > 0)
            return normal_pos;
        else
            return -1;
    } else // it is not a inserted statement, return its index as the original index
        return instrument_idx;
}

// account for transaction dependency
float dependency_analyzer::get_dependency_score(set<pair<int, int>>& blocking_tid_pairs)
{
    set<pair<int, int>> refined_graph;
    set<pair<int, int>> e2_refined_graph;
    set<int> refined_nodes;
    for (int i = 0; i < tid_num - 1; i++) // insert all txn id as nodes
    ...
    for (int i = 0; i < tid_num; i++) {
        for (int j = 0; j < tid_num; j++) {
            if (i == j)
                continue;
            auto& dependency_set = dependency_graph[i][j];
            if (dependency_set.count(WRITE_READ) == 0 &&
                dependency_set.count(READ_WRITE) == 0 &&
                dependency_set.count(WRITE_WRITE) == 0)
                continue;
            if (dependency_set.count(START_DEPEND) > 0)
                continue;
            
            refined_graph.insert(pair<int, int>(i, j));
        }
    }
    for (auto& pair:blocking_tid_pairs) 
        refined_graph.insert(pair);
    
    // just return E * (E / MAX_E_NUM), i.e. E*E/(V*(V-1))
    auto E = refined_graph.size();
    auto V = refined_nodes.size(); // V should be larger than 1 (i.e. V > 1)
    return 1.0 * E * E / (1.0 * V * (V - 1));
    
    // // using approximated von Neumann entropy
    // // EQ 26 in paper "Approximate von Neumann entropy for directed graphs"
    // // the smaller one is the more complex one
    // // get E2
    // for (auto& pair:refined_graph) {
    //     auto u = pair.first;
    //     auto v = pair.second;
    //     if (refined_graph.count(make_pair(v, u)) > 0) {
    //         e2_refined_graph.insert(pair);
    //         cerr << "E2 pair: " << u << "->" << v << endl;
    //     }
    // }
    
    // map<int, int> indegree;
    // map<int, int> outdegree;
    // for (auto idx : refined_nodes) {
    //     indegree[idx] = 0;
    //     outdegree[idx] = 0;
    // }
    // for (auto graph_edge : refined_graph) {
    //     outdegree[graph_edge.first]++;
    //     indegree[graph_edge.second]++;
    // }

    // float sum = 0.0;
    // for (auto& graph_edge : refined_graph) {
    //     int u = graph_edge.first;
    //     int v = graph_edge.second;
    //     float result = 1.0 * indegree[u] / (indegree[v] * outdegree[u] * outdegree[u]);
    //     sum = sum + result;
    // }
    // for (auto& graph_edge : e2_refined_graph) {
    //     int u = graph_edge.first;
    //     int v = graph_edge.second;
    //     float result = 1.0 / (outdegree[u] * outdegree[v]);
    //     sum = sum + result;
    // }
    // auto v = refined_nodes.size();
    // // return 1.0 - 1.0 / v - sum / (2.0 * v * v);
    // return sum / (2.0 * v); // normalize, lower is better
}

// // account for statememt dependency
// float dependency_analyzer::get_dependency_score()
// {
//     set<pair<int, int>> refined_graph;
//     set<int> refined_nodes;
//     int item_depend_num = 0;
//     int interleave_num = 0;
//     int final_num = 0;
//     for (auto iter = stmt_dependency_graph.begin(); iter != stmt_dependency_graph.end(); iter++) {
//         // only considering item-dependency
//         auto& depend_set = iter->second;
//         if (depend_set.count(WRITE_READ) == 0 && 
//             depend_set.count(READ_WRITE) == 0 && 
//             depend_set.count(WRITE_WRITE) == 0) {
//             continue;
//         }
//         item_depend_num++;

//         // skip if the two transaction are not interleaved
//         if (depend_set.count(START_DEPEND) > 0)
//             continue;
//         interleave_num++;
        
//         // skip if one of the statement is inserted statement whose instrument dependency is incorrect
//         auto& stmt_edge = iter->first;
//         auto first_stmt = stmt_edge.first;
//         auto first_idx = first_stmt.transfer_2_stmt_idx(f_txn_id_queue);
//         auto first_original_idx = check_original_stmt_pos(first_idx);
//         if (first_original_idx == -1)
//             continue;

//         auto second_stmt = stmt_edge.second;
//         auto second_idx = second_stmt.transfer_2_stmt_idx(f_txn_id_queue);
//         auto second_original_idx = check_original_stmt_pos(second_idx);
//         if (second_original_idx == -1)
//             continue;
        
//         // skip if the two are from the same transaction
//         if (f_txn_id_queue[first_original_idx] == f_txn_id_queue[second_original_idx])
//             continue;
//         final_num++;
        
//         refined_graph.insert(pair<int, int>(first_original_idx, second_original_idx));
//         refined_nodes.insert(first_original_idx);
//         refined_nodes.insert(second_original_idx);
//     }
//     cerr << "item_depend_num: " << item_depend_num << endl;
//     cerr << "interleave_num: " << interleave_num << endl;
//     cerr << "final_num: " << final_num << endl;

//     if (refined_nodes.size() == 0)
//         return 0;
...
//         sum = sum + result;
//     }
//     return sum / (2.0 * refined_nodes.size());
// }