...
#define MAX_TXN_NUM 9
#define MAX_TXN_STMT_NUM 8
...
void transaction_test::assign_txn_id()
{
    ...
    while (available_tid.empty() == false) {
        int tid;
        if (concurrent_tid.size() < dx(MAX_CONCURRENT_TXN_NUM)) {
...
void transaction_test::instrument_txn_stmts(bool only_item_dependency)
{
    instrumentor i(stmt_queue, tid_queue, db_schema, only_item_dependency);
...
void transaction_test::data_aware_mutation()
{
    auto da_mutator = new data_aware_mutator();
    da_mutator->mutate(db_schema, init_db_content, stmt_queue);
...
bool transaction_test::analyze_txn_dependency(shared_ptr<dependency_analyzer>& da, bool strict_instrument_order)
{
    ...
    da = make_shared<dependency_analyzer>(init_content_vector, // init_output 
                            ...
                            1, // primary_key_idx
                            0,
                            strict_instrument_order); // write_op_key_idx
...
int transaction_test::trans_test_unit(int stmt_pos, stmt_output& output, bool debug_mode)
{
    ...
    try {
        ...
        if (err.find("blocked") != string::npos) {
            const string self_id_begin = "self_pid->[";
            const string self_id_end = "]<-";
            auto self_pos_begin = err.find(self_id_begin, 0) + self_id_begin.size();
            auto self_pos_end = err.find(self_id_end, self_pos_begin);
            auto self_id = err.substr(self_pos_begin, self_pos_end - self_pos_begin);
            
            size_t cur_pos = 0;
            const string blocking_id_begin = "blocking_pid->[";
            const string blocking_id_end = "]<-";
            while (1) {
                auto found_pos = err.find(blocking_id_begin, cur_pos);
                if (found_pos == string::npos)
                    break;
                found_pos += blocking_id_begin.size();
                auto end_pos = err.find(blocking_id_end, found_pos);
                auto blocking_id = err.substr(found_pos, end_pos - found_pos);
                blocking_pid_pairs.insert(make_pair(blocking_id, self_id));
                cur_pos = end_pos;
            }
            return 0;
        }
...
void transaction_test::trans_test(bool debug_mode, bool need_reset)
{
    if (need_reset == true) {
        dut_reset_to_backup(test_dbms_info);
        blocking_pid_pairs.clear();
    }
...
bool transaction_test::multi_stmt_round_test()
{
    ...
    while (1) { // until there is not statement in the stmt path
        ...
        dut_get_content(test_dbms_info, trans_db_content); // collect final database information
...
void transaction_test::generate_txn_testcase()
{
    assign_txn_id();
    assign_txn_status();
    gen_txn_stmts();
...
int transaction_test::crash_test()
{   
    // set these flags true to make generation not execute the statements
    test_dbms_info.can_trigger_error_in_txn = true;
    test_dbms_info.ouput_or_affect_num = 0;
    // generate statements for txns, but not run the statements
    dut_get_content(test_dbms_info, init_db_content); // get initial database content
    db_schema = get_schema(test_dbms_info);
    ...
    instrument_txn_stmts(true); // only consider item dependency

    trans_test(true, false);

    for (int tid = 0; tid < trans_num; tid++) {
        auto pid = trans_arr[tid].dut->get_process_id();
        pid_2_txnid[pid] = tid;
    }
    
    for (auto& pair:blocking_pid_pairs) {
        auto tid_pair = make_pair(pid_2_txnid[pair.first], pid_2_txnid[pair.second]);
        blocking_tid_pairs.insert(tid_pair);
    }
    cerr << "blocking tid pairs: " << endl;
    for (auto& pair:blocking_tid_pairs) {
        cerr << pair.first << "->" << pair.second << endl;
    }

    shared_ptr<dependency_analyzer> da_results;
    if (analyze_txn_dependency(da_results, false)) {
        // trigger bugs
    }
    auto score = da_results->get_dependency_score(blocking_tid_pairs);
    da_results->print_dependency_graph();
...
transaction_test::transaction_test(dbms_info& d_info)
{
    ...
    stmt_num = 0;
    for (int i = 0; i < trans_num; i++) {
        trans_arr[i].stmt_num = dx(MAX_TXN_STMT_NUM) + 2; // at least 2: begin and commit
