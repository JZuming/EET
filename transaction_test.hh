...
public:
    ...
    set<pair<string, string>> blocking_pid_pairs; // blocking_pid->waiting_pid
    map<string, int> pid_2_txnid;
    set<pair<int, int>> blocking_tid_pairs;

    void assign_txn_id();
    void assign_txn_status();
    void gen_txn_stmts();
    void instrument_txn_stmts(bool only_item_dependency = false);
    void clean_instrument();
    void block_scheduling();

    bool change_txn_status(int tid, txn_status final_status);
    bool analyze_txn_dependency(shared_ptr<dependency_analyzer>& da, bool strict_instrument_order = true); // input da is empty; output the analyzed da
    ...
    void trans_test(bool debug_mode = true, bool need_reset = true);
    ...
    int oracle_test();
    int crash_test();
    void generate_txn_testcase();
    void save_test_case(string dir_name);

    void data_aware_mutation();
};
